<template>
  ...
  <section class="contenedor_section_tabla">
      <div v-for="(rol_usuario, index) in roles_usuario" :key="index" class="card">
        <RouterLink class="contenedor_router" :to="{ path: '/rol_usuario/' + rol_usuario.id_usuario + '/detalles' }">
          <button type="button" class="boton_card">
            <div class="bloque_datos">
              <p class="p_valor"><i class="ri-mail-fill icon_card"></i> <strong class="p_dato">Correo:</strong> {{ rol_usuario.correo }}</p>
              <p class="p_valor"><i class="ri-account-circle-fill icon_card"></i> <strong class="p_dato">Rol:</strong> {{ roles[rol_usuario.fk_id_rol] }}</p>
            </div>
          </button>
        </RouterLink>
      </div>
