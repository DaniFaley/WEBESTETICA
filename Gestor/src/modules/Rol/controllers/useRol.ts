...
export const useRol = () =>{
    ...
    const agregarRol = async (rols:RolAgregar) => {
    const respuesta = await rolApi.post('/', rols)
        if(respuesta.data.affectedRows >= 1) {
            mensaje.value = 1;
        }
    }
    //Modificar registro
    const actualizarRol = async (rols:Rol) => {
        const respuesta = await rolApi.put('/',rols)
        if(respuesta.data.affectedRows >= 1) {
            mensaje.value = 1;
        }
    }
    //Eliminar registro
    const borrarRol = async (rols:Rol) => {
        const respuesta = await rolApi.delete('/',{data: {id_rol: rols.id_rol}})
